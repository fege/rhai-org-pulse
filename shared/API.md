...
| `scope-registry` | `{ createScopeRegistry }` — dynamic scope registry for API tokens. Modules register scopes via `context.registerScopes()`. Methods: `register(config)`, `isValid(key)`, `getAll()`, `getValidKeys()`. |
